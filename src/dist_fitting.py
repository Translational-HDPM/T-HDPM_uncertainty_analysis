...
def test_negative_binomial_fit(
    data_arg: Sequence[float],
) -> tuple[float, float, float, float, float]:
    """
...
def test_zero_inflated_poisson_fit(
    data_arg: Sequence[float],
) -> tuple[float, float, float, float, float]:
    """
    ...
    """
    data = pd.Series(np.asarray(data_arg, dtype=np.int64))
    zip_model = cm.ZeroInflatedPoisson(
        endog=data,
...
def test_zero_inflated_negative_binomial_fit(
    data_arg: Sequence[float],
    ...
    alpha, lambda_zinb, gamma = (
        zinb_results.params["alpha"],
