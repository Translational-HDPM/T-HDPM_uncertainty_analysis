...
from classifier import get_true_label, run_sim_one_patient, cl_score, linear_score

sns.set_style("ticks")

def plot_uncert_at_thresh(Patients_df_zScore, coefficients, labels,
    ...
    for i in range(243):
        scores = run_sim_one_patient(Patients_df_zScore.iloc[:, i], 
                                    uncertainty, num_runs, coefficients, seed)
        ...
        for j in range(len(x_data)):
            if y_true == 1 and y_data[j] < thresh:
                colour[j] = 1
                false_neg = false_neg + 1
                num_subj_unreliable[i] = 1
            elif y_true == 0 and y_data[j] > thresh:
                colour[j] = 2
                false_pos = false_pos + 1
                num_subj_unreliable[i] = 1
            elif y_true == 1 and y_data[j] > thresh:
                true_pos = true_pos +1
            elif y_true == 0 and y_data[j] < thresh:
                true_neg = true_neg +1  
        plt.scatter(x_data, y_data, c=colour, 
                    cmap = 'Dark2_r' ,alpha = 0.15, s=100)
    ...
    plt.text(thresh/2, 0.1, 'TN', fontsize = 20)
    plt.text(thresh+(1-thresh)/2, 0.1, 'FN', fontsize=20)
    plt.text(thresh/2, 0.9, 'FP', fontsize =20)
    plt.text(thresh+(1-thresh)/2, 0.9, 'TP', fontsize=20)
    plt.tick_params(labelsize=18)

    fig.savefig(f"uncert_around_thresh_{thresh}_uncert_{uncertainty}pc.png")

    return true_pos, true_neg, false_pos, false_neg, np.sum(num_subj_unreliable)

def sub_accuracy(Patients_df_zScore, coefficients,
                uncertainty_range, thresh, num_runs, seed):
    ...
    for i in range(len(uncertainty_range)):
        ...
        false_pos_df[false_pos_df.columns[i]] = \
                [sum_FP, sum_FN, sum_TP, sum_TN, unreliable_subjects]
...
def sub_accuracy_smoothed_plot(Patients_df_zScore, coefficients,
                uncertainty_range, thresh, num_runs, seed):
    """The purpose of this function is to calculate 2 things based on %RSD (uncertainty_range), threshold (thresh), number of MC simulations (num_runs) :
    1. subject accuracy based on number of simulations : this info is provided in a dataframe called "accuracy_df"
    2. True Positives/Negatives, False Positves/Negatives: this info is provided in a dtatframe called "false_pos_df"
    ...
    This function generates a V-plot based on a smoother curve compared to the original.
    """
    real_score = np.zeros(243)
    accuracy = np.zeros(243)
    ...
    NCI = 0
    accuracy_df= pd.DataFrame(columns= uncertainty_range)
    accuracy_smoothed_df= pd.DataFrame(columns= uncertainty_range)

    smth_lower_bnd = max(thresh-0.25, 0)
    smth_upper_bnd = min(thresh+0.25, 1)
    ...
    for i in range(len(uncertainty_range)):
        ...
        if uncertainty_range[i] > 15:
            bounds_mask = (real_score>=smth_lower_bnd) & \
                            (real_score<=smth_upper_bnd)
            smoothed[bounds_mask] = lowess(accuracy[bounds_mask],
                                           real_score[bounds_mask],
                                           frac=0.15, it=3,
                                           return_sorted=False)
        accuracy_smoothed_df[accuracy_df.columns[i]] = smoothed * 100
    ...
    Sub_score = pd.Series(real_score)
    accuracy_df['Classifier Score']= Sub_score
    accuracy_smoothed_df['Classifier Score'] = Sub_score
    
    dfm = accuracy_smoothed_df.melt('Classifier Score', 
                var_name='%RSD', value_name='Agreement')   
    fig = sns.relplot(x= "Classifier Score" , y="Agreement", 
                      hue='%RSD', data=dfm, kind='line', 
                      height=10, aspect=1, legend=False,
                      linewidth=1, palette="rainbow")
    sns.rugplot(data=dfm, x="Classifier Score")
    plt.xlabel("Classifier score", fontsize= 24)
    plt.ylabel("Agreement", fontsize = 24)
    plt.axis([0.0, 1.0, 40.0, 100.0])
    plt.legend(title="% RSD", title_fontsize=18,
               ...
               loc="best", fontsize=18)
    fig.tick_params(labelsize=18)
    fig.savefig(f"v_plot_smoothed_thres_{thresh}.png")
